...
The authors first consider a problem: *How do we classifcy nodes in a graph, where labels are only available for a small subset of nodes?* Think of a citation network where some documents have labels, but most do not. The authors frame it as "graph-based semi-supervised learning, where label information is smoothed over the graph via some form explicit graph-based regularization term in the loss function."
...
$$
\begin{aligned}
\mathcal{L} &= \mathcal{L}_0 + \lambda \mathcal{L}_{reg} \text{, where} \\

\mathcal{L}_{reg} &= \sum_{i,j} A_{i,j} \lVert f(X_{i}) - f(X_{j}) \rVert ^{2} = f(X)^{T} \Delta f(X)
\end{aligned}
$$

* $\mathcal{L}_0$ : supervised loss with respect to labeled part of graph
* $\mathcal{L}_{reg}$ : graph Laplacian regularization term
* $f(\dots)$ : a neural network-like differentiable function
* $\lambda$ : regularization weight
* $X$ : matrix of node feature vectors $X_{i}$
* $\Delta = D - A$ : unnormalized [graph Laplacian](https://en.wikipedia.org/wiki/Laplacian_matrix) of an undirected graph $G=(V, E)$ , with $N$ nodes $v_{i} \in V$ , edges $(v_{i}, v_{j}) \in E$
...
For all nodes with labels, train on a supervised target $\mathcal{L}_0$ and avoid graph regularization $\mathcal{L}_{reg}$ in the loss function.
...
* This propagation rule comes from first-order approximation of localed sectral filters on graphs
...
* eigendecomposition of $L_{\text{norm}}$ is expensive for large graphs
We can approxmate $g_{\theta}(\Lambda)$ using a truncated expansion in terms of Chebyshev polynomials $T_{k}(x)$ up to $K^{\text{th}}$ order:
...
* Complexity of calcuating this is $\mathcal{O}(\lvert \mathcal{E} \rvert)$ , linear in the number of edges
...
Such model can alleviate problem of overfitting on local neighborhood structures for graphs with very wide node degree distributions, i.e. some nodes are highly connected and some nodes are barely connected.
...
To address overfitting and minimize operations per layer, we constrain the number of parameters:
...
Consider a two-layer GCN for semi-supervised node classification on a graph with symmeteric adjacency matrix $A$ (binary or weighted).
