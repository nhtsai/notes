...
* $\Delta = D - A$: un-normalized [graph Laplacian](https://en.wikipedia.org/wiki/Laplacian_matrix) of an undirected graph $G=(V, E)$, with $N$ nodes $v\_{i} \in V$, edges $(v\_{i}, v\_{j}) \in E$
    ...
    * $D\_{ii} = \displaystyle\sum\_{j} A\_{ij}$: degree matrix
...
* $\tilde{D}\_{ii} = \displaystyle\sum\_{j} \tilde{A}\_{ij}$: degree matrix of $\tilde{A}$
...
The approximated spectral graph convolution is now: $g\_{\theta'} \star x \approx \theta'\_{0}x - \theta'\_{1} D^{-\frac{1}{2}} A D^{-\frac{1}{2}} x \text{, where } \theta'\_{0}, \theta'\_{1} \text{ are filter parameters}$.
...
* $\tilde{D}\_{ii} = \displaystyle\sum\_{j} \tilde{A}\_{ij}$: adjusted degree matrix
...
Preprocessing step: $\hat{A} = \tilde{D}^{-\frac{1}{2}} \tilde{A} \tilde{D}^{-\frac{1}{2}}$.

2-Layer Forward propagation step: $Z = f(X,A) = \operatorname{softmax}(\hat{A} (\overbrace{\operatorname{ReLU}(\hat{A} X W^{(0)})}^{\text{first layer}}) W^{(1)})$.
...
* $\operatorname{softmax}(x\_{i}) = \frac{1}{z} \exp{x\_{i}}$: softmax activation function
    * $z = \displaystyle\sum\_{i} \exp{x\_{i}}$: applied row-wise

Evaluate cross-entropy error over all labeled examples: $\mathcal{L} = - \displaystyle\sum\_{l \in \mathcal{Y}\_{L}} \displaystyle\sum\_{f=1}^{F} Y\_{lf} \ln{Z\_{lf}}$.
* $\mathcal{Y}\_{L}$: set of node indices that have labels
* $F$: number of output labels
* $Y\_{lf}$: correct value for node index $l$ and output label $f$
    * Should be 0 for incorrect output labels, 1 for correct output label
* $Z\_{lf}$: predicted value for node index $l$ and output label $f$
