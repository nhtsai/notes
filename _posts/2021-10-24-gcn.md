...
where $\mathcal{L}_{reg} = \sum_{i,j} A_{i,j} \lVert f(X_{i}) - f(X_{j}) \rVert ^{2} = f(X)^{T} \Delta f(X)$
...
For all nodes with labels, train on a supervised target $\mathcal{L}_{0}$ and avoid graph regularization $\mathcal{L}_{reg}$ in the loss function.
